...
<div class="container mx-auto px-4 py-8">
        ...
        <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 p-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-4xl font-bold text-gray-900">Modifier une Phase</h1>
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
        <form action="{{ route('pdfcs.phases.update', [$pdfc, $phase]) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           ...
                           id="date" 
                           name="date" 
                           value="{{ old('date', $phase->date ? $phase->date->format('Y-m-d') : '') }}">
                    @error('date')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           ...
                           id="date_de_début" 
                           name="date_de_début" 
                           value="{{ old('date_de_début', $phase->date_de_début ? $phase->date_de_début->format('Y-m-d') : '') }}">
                    @error('date_de_début')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           ...
                           id="date_de_fin" 
                           name="date_de_fin" 
                           value="{{ old('date_de_fin', $phase->date_de_fin ? $phase->date_de_fin->format('Y-m-d') : '') }}">
                    @error('date_de_fin')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="date" 
                           ...
                           id="échéance" 
                           name="échéance" 
                           value="{{ old('échéance', $phase->échéance ? $phase->échéance->format('Y-m-d') : '') }}">
                    @error('échéance')
                        <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                    @enderror
                </div>
