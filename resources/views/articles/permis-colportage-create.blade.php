...
@section('title', 'Créer un Permis de Colportage - DEFATP')
                ...
                <form action="{{ route('articles.store-permis-colportage', $article) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-6 mb-6">
                        <x-form-section title="Permis d'Enlever" icon="fas fa-truck-loading" color="green" columns="1">
                            ...
                            @if(isset($carnetsDisponibles) && $carnetsDisponibles->isNotEmpty())
                                <div class="form-group mt-4">
                                    <label for="carnet_id" class="block text-sm font-semibold text-gray-700 mb-2">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Obligatoire. Le numéro choisi sera marqué « épuisé » après enregistrement.</p>
                                </div>
                            @else
                                <x-alert type="warning" title="Aucun carnet disponible" class="mt-4">
                                    Aucun numéro de carnet « disponible » n'est actuellement disponible. Veuillez en créer un avant de générer un permis de colportage.
                                </x-alert>
                            @endif
                        </x-form-section>
                        ...
                        <div class="mt-4">
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                Optionnel. Vous pouvez joindre une copie scannée du permis ou tout document justificatif.
                            </p>
                        </div>
                    ...
                    <div class="flex justify-end gap-4 pt-6 border-t border-green-200">
                        <a href="{{ route('articles.permis-colportage', array_filter([$article, 'permis_enlever_id' => $selectedPermisEnleverId])) }}"
                           class="inline-flex items-center gap-2 px-6 py-3 border border-green-300 rounded-xl text-green-700 hover:bg-green-50 transition-all">
