...
<div class="min-w-0 max-w-full overflow-x-hidden">
    ...
    <!-- Info Details Card -->
    <div class="rounded-2xl border bg-white mb-6" style="border-color: rgba(154,179,163,0.4); box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-5">
                ...
                <div>
                    ...
                    <dd class="text-sm font-semibold text-gray-900">{{ $cession->type === 'appel_offre' ? "Appel d'offre" : 'Adjudication' }}</dd>
                </div>
                <div>
                ...
                <div>
                    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Date d'adjudication</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ optional($cession->date_adjudication)->format('d/m/Y') ?? '-' }}</dd>
                </div>
                @else
                <div>
                ...
                <div>
                    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Date d'attribution</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ optional($cession->date_attribution)->format('d/m/Y') ?? '-' }}</dd>
                </div>
                @endif
                <div>
                    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">Statut</dt>
                    <dd>
                        <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold {{ $badge['bg'] }} {{ $badge['text'] }} ring-1 {{ $badge['ring'] }}">
    ...
    <div class="rounded-2xl border bg-white overflow-hidden" style="border-color: rgba(154,179,163,0.4); box-shadow: 0 2px 8px rgba(0,0,0,0.04);">
        <div class="px-6 py-4 border-b flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3" style="border-color: rgba(154,179,163,0.3);">
            <div class="flex items-center gap-3">
        ...
        <div class="overflow-x-auto w-full" style="-webkit-overflow-scrolling: touch;">
            <table id="cessionArticlesTable" data-table="cessionArticlesTable" class="w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    ...
                    @php
                        $stepLabels = [
                            'cahier_affiche'     => ['label' => 'Cahier & Affiche',      'class' => 'bg-gray-100 text-gray-700'],
                            'contrat_vente'      => ['label' => 'Contrat de vente',      'class' => 'bg-green-100 text-green-700'],
                            'paiement_charges'   => ['label' => 'Paiement charges',      'class' => 'bg-blue-100 text-blue-700'],
                            'paiement_tranches'  => ['label' => 'Paiement tranches',     'class' => 'bg-purple-100 text-purple-700'],
                            'recollement'        => ['label' => 'Récollement',           'class' => 'bg-orange-100 text-orange-700'],
                            'main_levee'         => ['label' => 'Main levée',            'class' => 'bg-emerald-100 text-emerald-700'],
                        ];
                        $currentStep = $article->current_step ?? 'cahier_affiche';
                        $stepInfo = $stepLabels[$currentStep] ?? ['label' => ucfirst(str_replace('_', ' ', $currentStep)), 'class' => 'bg-gray-100 text-gray-700'];
                    @endphp
                    ...
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-xs text-gray-400 mb-4">Commencez par ajouter un premier article à cette cession.</p>
                                <a href="{{ route('articles.create', ['cession_id' => $cession->id]) }}"
