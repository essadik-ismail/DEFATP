...
$stateOrder = array_keys(WF::LABELS);
$workflowStateIndex = array_search($wfState, $stateOrder, true);
$letterSignedIndex = array_search(WF::LETTER_SIGNED_UPLOADED, $stateOrder, true);
$permitIssuedIndex = array_search(WF::PERMIT_ISSUED, $stateOrder, true);
...
$allStepDefs = [
    ...
    WF::LETTER_GENERATED     => ['icon' => 'fa-envelope',       'title' => 'Lettre adjudicataire',      'badge' => '4'],
    WF::LETTER_SIGNED_UPLOADED => ['icon' => 'fa-file-signature','title' => 'Lettre signée',            'badge' => '5'],
    WF::CAUTION_PAID         => ['icon' => 'fa-shield-alt',     'title' => 'Paiement caution',          'badge' => '6'],
    WF::TAXES_PAID           => ['icon' => 'fa-money-bill-wave','title' => 'Paiement des taxes',        'badge' => '7'],
    WF::PERMIT_READY         => ['icon' => 'fa-file-alt',       'title' => 'Permis d\'exploiter',       'badge' => '8'],
    WF::PERMIT_ISSUED        => ['icon' => 'fa-stamp',          'title' => 'Permis émis',               'badge' => '9'],
    WF::PV_INSTALLATION_DONE => ['icon' => 'fa-clipboard-check','title' => 'PV d\'installation',        'badge' => '10'],
    WF::VEHICLES_DECLARED    => ['icon' => 'fa-truck',          'title' => 'Déclaration véhicules',     'badge' => '11'],
    WF::TRANCHES_IN_PROGRESS => ['icon' => 'fa-credit-card',    'title' => 'Paiement des tranches',     'badge' => '12'],
    WF::COLPORTAGE_ACTIVE    => ['icon' => 'fa-shipping-fast',  'title' => 'Colportage',                'badge' => '13'],
    WF::RECOLEMENT_PENDING   => ['icon' => 'fa-clipboard-list', 'title' => 'Récolement',                               'badge' => '14'],
    WF::MAINLEVEE_DONE       => ['icon' => 'fa-unlock',         'title' => 'Mainlevée',                 'badge' => '15'],
    WF::CLOSED               => ['icon' => 'fa-archive',        'title' => 'Clôture',                   'badge' => '16'],
];

$hiddenShowStates = [WF::LETTER_SIGNED_UPLOADED, WF::PERMIT_ISSUED, WF::VEHICLES_DECLARED];
...
if (isset($displaySteps[WF::LETTER_GENERATED])) {
    $displaySteps[WF::LETTER_GENERATED]['label'] = 'Lettre adjudicataire';

    if ($workflowStateIndex !== false && $letterSignedIndex !== false && $workflowStateIndex >= $letterSignedIndex) {
        $displaySteps[WF::LETTER_GENERATED]['status'] = 'done';
        $displaySteps[WF::LETTER_GENERATED]['blocked_reason'] = null;
    } elseif ($wfState === WF::LETTER_GENERATED && isset($displaySteps[WF::LETTER_SIGNED_UPLOADED])) {
        $displaySteps[WF::LETTER_GENERATED]['status'] = $displaySteps[WF::LETTER_SIGNED_UPLOADED]['status'];
        $displaySteps[WF::LETTER_GENERATED]['blocked_reason'] = $displaySteps[WF::LETTER_SIGNED_UPLOADED]['blocked_reason'];
    }
}

unset($displaySteps[WF::LETTER_SIGNED_UPLOADED]);

if (isset($displaySteps[WF::PERMIT_READY])) {
    $displaySteps[WF::PERMIT_READY]['label'] = "Permis d'exploiter";

    if ($workflowStateIndex !== false && $permitIssuedIndex !== false && $workflowStateIndex >= $permitIssuedIndex) {
        $displaySteps[WF::PERMIT_READY]['status'] = 'done';
        $displaySteps[WF::PERMIT_READY]['blocked_reason'] = null;
    } elseif ($wfState === WF::PERMIT_READY && isset($displaySteps[WF::PERMIT_ISSUED])) {
        $displaySteps[WF::PERMIT_READY]['status'] = $displaySteps[WF::PERMIT_ISSUED]['status'];
        $displaySteps[WF::PERMIT_READY]['blocked_reason'] = $displaySteps[WF::PERMIT_ISSUED]['blocked_reason'];
    }
}

unset($displaySteps[WF::PERMIT_ISSUED]);

$visibleStepDefs = [];
$visibleBadge = 1;

foreach ($allStepDefs as $state => $def) {
    if (in_array($state, $hiddenShowStates, true)) {
        continue;
    }

    $def['badge'] = (string) $visibleBadge++;
    $visibleStepDefs[$state] = $def;
}

$stateKeys  = array_keys($visibleStepDefs);
$activeKey  = array_search($wfState, $stateKeys);
$initialVisibleState = collect($stateKeys)->first(function ($state) use ($displaySteps) {
    return ($displaySteps[$state]['status'] ?? null) === 'active';
});

if (!$initialVisibleState) {
    $initialVisibleState = isset($visibleStepDefs[$wfState]) ? $wfState : null;
}

if (!$initialVisibleState) {
    $initialVisibleState = collect($stateKeys)->first(function ($state) use ($displaySteps) {
        return ($displaySteps[$state]['status'] ?? null) === 'blocked';
    });
}
    ...
    @foreach($visibleStepDefs as $state => $def)
        ...
        <div x-show="active === '{{ $state }}'"
            ...
            <div class="p-4 sm:p-5">
                ...
                {{-- ============================================================
                     Step 1 — DRAFT_ARTICLE
                ============================================================ --}}
                @if($state === WF::DRAFT_ARTICLE)
                    @if(!$contractVente)
                        @php $validateBlocked = true; $validateBlockedReason = 'Un contrat de vente doit être créé avant de valider cette étape.'; @endphp
                    @endif
                ...
                {{-- ============================================================
                     Step 4 — LETTER_GENERATED
                ============================================================ --}}
                @elseif($state === WF::LETTER_GENERATED)
                    @if(!$contractVente)
                        <p class="text-sm text-gray-400 italic">Contrat requis d'abord.</p>
                    @else
                                        ...
                                        <tr>
                                            ...
                                            <td class="py-2 pr-4">
                                                @if($cautionPayment?->is_paye)
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 rounded-full text-xs font-medium"><i class="fas fa-check-circle"></i> Payé</span>
                                                @else
                                                    <span class="inline-flex items-center gap-1 px-2 py-0.5 bg-red-100 text-red-700 rounded-full text-xs font-medium"><i class="fas fa-times-circle"></i> Impayé</span>
                                                @endif
                                            </td>
                                            <td class="py-2">
                ...
                {{-- ============================================================
                     Step 8 — PERMIT_READY
                ============================================================ --}}
                @elseif($state === WF::PERMIT_READY)
                    @php
                        $serviceRenduPaid = !$article->service_rendu_anef || $article->date_payement_service_anef;
                    @endphp
                    @if(!$allTaxesPaid || !$cautionPaid)
                        @php $validateBlocked = true; $validateBlockedReason = 'La caution et toutes les taxes doivent être payées avant de valider.'; @endphp
                    @elseif(!$serviceRenduPaid)
                        @php $validateBlocked = true; $validateBlockedReason = 'Le service rendu par l\'ANEF doit être payé avant de valider.'; @endphp
                    ...
                    @if(!$allTaxesPaid || !$cautionPaid)
                        <div class="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg text-sm text-amber-800">
                            ...
                            <span>La caution et toutes les taxes doivent être payées pour déverrouiller cette étape.</span>
                        </div>
                    @elseif($permisExploiter)
                        <div class="info-tiles grid grid-cols-2 md:grid-cols-3 gap-3 text-sm mb-4">
                            @foreach([
                ...
                {{-- ============================================================
                     Step 9 — PERMIT_ISSUED
                ============================================================ --}}
                @elseif($state === WF::PERMIT_ISSUED)
                    @if(!$permisExploiter)
                        @php $validateBlocked = true; $validateBlockedReason = 'Le permis d\'exploiter doit être émis avant de valider.'; @endphp
                    @endif
                    @if($permisExploiter)
                        <div class="info-tiles grid grid-cols-2 md:grid-cols-3 gap-3 text-sm mb-4">
                            @foreach([
                    ...
                    @else
                        <p class="text-sm text-gray-400 italic mb-3">Permis d'exploiter non encore émis.</p>
                        <a href="{{ route('articles.permis-exploiter', $article) }}"
                           class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors">
                            <i class="fas fa-plus"></i> Créer le permis d'exploiter
                        </a>
                    @endif
                ...
                {{-- ============================================================
                     Step 11 — VEHICLES_DECLARED
                ============================================================ --}}
                @elseif($state === WF::VEHICLES_DECLARED)
                    @if($vehicleDeclarations->isEmpty())
                        @php $validateBlocked = true; $validateBlockedReason = 'Au moins un véhicule doit être déclaré avant de valider.'; @endphp
                    @endif
                    @if($vehicleDeclarations->isNotEmpty())
                        <div class="overflow-x-auto mb-4">
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="border-b border-gray-200">
                                        <th class="text-left py-2 pr-4 text-gray-600 font-medium">Immatriculation</th>
                                        <th class="text-left py-2 pr-4 text-gray-600 font-medium">Marque</th>
                                        <th class="text-left py-2 pr-4 text-gray-600 font-medium">Capacité</th>
                                        <th class="text-left py-2 pr-4 text-gray-600 font-medium">Chauffeur</th>
                                        <th class="text-left py-2 text-gray-600 font-medium">Date</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-100">
                                    @foreach($vehicleDeclarations as $veh)
                                    <tr class="hover:bg-gray-50">
                                        <td class="py-2 pr-4 font-medium">{{ $veh->immatriculation }}</td>
                                        <td class="py-2 pr-4">{{ $veh->marque ?? '—' }}</td>
                                        <td class="py-2 pr-4">{{ $veh->capacite ? $veh->capacite . ' ' . $veh->capacite_unite : '—' }}</td>
                                        <td class="py-2 pr-4">{{ $veh->chauffeur_nom ?? '—' }}</td>
                                        <td class="py-2">{{ $veh->date_declaration?->format('d/m/Y') ?? '—' }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                    ...
                    @else
                        <p class="text-sm text-gray-600 mb-4">Aucun véhicule déclaré.</p>
                    @endif
                    ...
                    @if($colportageIsNextAction)
                        <div class="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center justify-between gap-3">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-sm font-semibold text-amber-800">Un permis d'enlever a été créé. Vous pouvez maintenant créer un permis de colportage.</span>
                            </div>
                            <button type="button"
                ...
                @elseif($state === WF::COLPORTAGE_ACTIVE)
                    ...
                    @if($permisEnleverList->isEmpty())
                        <div class="text-sm text-gray-500 mb-4">
                            ...
                            <a href="{{ route('articles.permis-enlever', $article) }}" class="text-emerald-600 hover:underline ml-1">Créer un permis d'enlever</a>
                        </div>
                    @else
                                        ...
                                        <table class="w-full text-xs divide-y divide-gray-100">
                                            <thead class="bg-emerald-50 text-emerald-800 uppercase font-semibold">
                                                <tr>
                                                    ...
                                                    <th class="px-3 py-2 text-left">Date début</th>
                ...
                @php
                    ...
                    $showTransitionButton = $isActive
                        ...
                        && $transitionTargetState !== WF::LETTER_SIGNED_UPLOADED
                        && $transitionTargetState !== WF::PERMIT_ISSUED;
                            ...
                            <div class="flex items-center gap-3 p-4 rounded-xl"
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-emerald-800 mb-0.5">
                                        {{ $isValidatingCurrentStep ? 'Étape à valider' : 'Prochaine étape' }}
                                    </p>
                                ...
                                <form action="{{ route('workflow.transition', $article) }}" method="POST" class="flex-shrink-0">
                                    ...
                                    <button type="submit"
                                            onclick="return confirm('Attention : cette action est irréversible. Une fois validé, cet article ne pourra plus être modifié. Confirmez-vous ?')"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ Permis d'Enlever selector in Step 13: reload page with selected PE ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬
    ...
    // ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ Live search inside colportage table ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚ÂÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬
