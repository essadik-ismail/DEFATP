...
@section('title', 'Modifier véhicule - DEFATP')

@section('breadcrumb')
<li class="breadcrumb-item"><a href="{{ route('vehicles.overview') }}">Véhicules</a></li>
...
<div class="min-w-0 max-w-full overflow-x-hidden">
    <div class="container mx-auto px-4 max-w-3xl">
        ...
        <x-page-header
            ...
            :subtitle="'Immatriculation : ' . $vehicle->immatriculation"
            icon="fas fa-truck"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <form action="{{ route('vehicles.standalone.update', $vehicle) }}" method="POST" class="space-y-6">
                ...
                <div class="rounded-lg border px-4 py-3 text-sm {{ $vehicle->contractVente ? 'border-green-100 bg-green-50 text-green-900' : 'border-blue-100 bg-blue-50 text-blue-900' }}">
                    @if($vehicle->contractVente?->article)
                        Lié à l'article #{{ $vehicle->contractVente->article->numero ?? $vehicle->contractVente->article->id }}
                    @elseif($vehicle->contractVente)
                        Lié au contrat #{{ $vehicle->contractVente->id }}
                    @else
                        Véhicule indépendant, sans contrat ni article.
                    @endif
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="capacite" id="capacite"
                               value="{{ old('capacite', $vehicle->capacite) }}" step="0.01" min="0"
                               class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="capacite_unite" id="capacite_unite"
                            ...
                            @foreach(['m3' => 'm³', 'stere' => 'Stère', 'sacs' => 'Sacs', 'tonnes' => 'Tonnes', 'autre' => 'Autre'] as $val => $label)
                                <option value="{{ $val }}" {{ old('capacite_unite', $vehicle->capacite_unite) === $val ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="chauffeur_nom" id="chauffeur_nom"
                               value="{{ old('chauffeur_nom', $vehicle->chauffeur_nom) }}"
                               class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                               maxlength="150">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="chauffeur_cin" id="chauffeur_cin"
                               value="{{ old('chauffeur_cin', $vehicle->chauffeur_cin) }}"
                               class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500"
                               maxlength="50">
                    </div>
                    ...
                    <div class="form-group md:col-span-2">
                        ...
                        <input type="date" name="date_declaration" id="date_declaration"
                               value="{{ old('date_declaration', $vehicle->date_declaration?->format('Y-m-d')) }}"
                               class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">
                    </div>
                </div>
