...
@section('title', 'Modifier ODF - Étape 4 - DEFATP')
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl p-8 border border-white/20">
        <form action="{{ route('odfs.update.step4', $odf) }}" method="POST" id="etapsForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                <div class="mb-4 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">Modifiez ou ajoutez des étapes pour cette ODF.</p>
                        </div>
                    </div>
                    <button type="button" onclick="addEtapRow()" class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-300">
                ...
                <div id="etaps-container" class="space-y-4">
                    @forelse($etaps as $index => $etap)
                        @php
                            $participants = $etap->participants ? json_decode($etap->participants, true) : [];
                        @endphp
                        <div class="etap-row bg-green-50 border border-green-200 rounded-2xl p-4">
                            <div class="flex items-center justify-between mb-4">
                                <h3 class="text-md font-semibold text-green-700">Étape #{{ $index + 1 }}</h3>
                                <label class="inline-flex items-center text-red-600 hover:text-red-800 text-sm cursor-pointer">
                                    <input type="checkbox" name="etaps_to_delete[]" value="{{ $etap->id }}" class="mr-2">
                                    <i class="fas fa-trash mr-1"></i>Supprimer
                                </label>
                            </div>
                            <input type="hidden" name="odf_etaps[{{ $index }}][id]" value="{{ $etap->id }}">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Objet</label>
                                    <input type="text" name="odf_etaps[{{ $index }}][objet]" class="form-input w-full" placeholder="Objet de l'étape" value="{{ old("odf_etaps.$index.objet", $etap->objet) }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Lieu</label>
                                    <input type="text" name="odf_etaps[{{ $index }}][lieu]" class="form-input w-full" placeholder="Lieu" value="{{ old("odf_etaps.$index.lieu", $etap->lieu) }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Date</label>
                                    <input type="date" name="odf_etaps[{{ $index }}][date]" class="form-input w-full" value="{{ old("odf_etaps.$index.date", $etap->date) }}">
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Type</label>
                                    <input type="text" name="odf_etaps[{{ $index }}][type]" class="form-input w-full" placeholder="Type d'étape" value="{{ old("odf_etaps.$index.type", $etap->type) }}">
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Description</label>
                                    <textarea name="odf_etaps[{{ $index }}][description]" rows="3" class="form-input w-full" placeholder="Description de l'étape">{{ old("odf_etaps.$index.description", $etap->description) }}</textarea>
                                </div>
                                <div class="md:col-span-2">
                                    <label class="block text-sm font-semibold text-gray-700 mb-1">Résultat</label>
                                    <textarea name="odf_etaps[{{ $index }}][resultat]" rows="3" class="form-input w-full" placeholder="Résultat de l'étape">{{ old("odf_etaps.$index.resultat", $etap->resultat) }}</textarea>
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @forelse($diagnostics as $diag)
                                            <label class="inline-flex items-center px-3 py-1 rounded-full border border-green-300 bg-green-50 text-xs text-green-800 cursor-pointer">
                                                <input type="checkbox"
                                                       class="mr-2"
                                                       name="odf_etaps[{{ $index }}][participants][]"
                                                       value="{{ $diag->id }}"
                                                       {{ in_array($diag->id, $participants) ? 'checked' : '' }}>
                    ...
                    @empty
                        <div class="etap-row bg-green-50 border border-green-200 rounded-2xl p-4">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <button type="button" class="remove-etap hidden text-red-600 hover:text-red-800 text-sm" onclick="removeEtapRow(this)">
                                    <i class="fas fa-trash mr-1"></i>Supprimer
                                </button>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        @forelse($diagnostics as $diag)
                                            <label class="inline-flex items-center px-3 py-1 rounded-full border border-green-300 bg-green-50 text-xs text-green-800 cursor-pointer">
                                                <input type="checkbox" class="mr-2" name="odf_etaps[0][participants][]" value="{{ $diag->id }}">
                                                <span>Diag #{{ $diag->id }} - {{ $diag->type }}@if($diag->nom) - {{ $diag->nom }}@endif</span>
                                            </label>
                                        @empty
                                            <p class="text-xs text-gray-500">Aucun diagnostic saisi pour le moment.</p>
            ...
            <div class="flex items-center justify-between pt-6 border-t border-gray-200 mt-6">
                <a href="{{ route('odfs.edit.step3', $odf) }}" class="inline-flex items-center gap-2 px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 transition-all duration-300">
                    <i class="fas fa-arrow-left"></i>
                    <span>Retour étape 3</span>
                </a>

                <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-300 transform hover:scale-105 shadow-lg">
...
function addEtapRow() {
    ...
    row.innerHTML = `
        <div class="flex items-center justify-between mb-4">
            ...
            <button type="button" class="remove-etap text-red-600 hover:text-red-800 text-sm" onclick="removeEtapRow(this)">
                <i class="fas fa-trash mr-1"></i>Supprimer
            </button>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
