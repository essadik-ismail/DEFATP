...
<div class="min-w-0 max-w-full overflow-x-hidden">
    ...
    <x-page-header
        title="Créer un rôle"
        subtitle="Définir un nouveau rôle et lui assigner des permissions"
    ...
    <form action="{{ route('roles.store') }}" method="POST" novalidate>
        ...
        <div style="display:grid; grid-template-columns:1fr; gap:1rem; max-width:1000px;">
...
function selectAll() {
    ...
    document.querySelectorAll('[id^="module_"]:not([id^="module_body"]):not([id^="chevron"])').forEach(cb => { if (cb.type === 'checkbox') cb.checked = true; });
    updateCounter();
}
function deselectAll() {
    document.querySelectorAll('.perm-check').forEach(cb => { cb.checked = false; });
    document.querySelectorAll('[id^="module_"]:not([id^="module_body"]):not([id^="chevron"])').forEach(cb => { if (cb.type === 'checkbox') cb.checked = false; });
    updateCounter();
}
