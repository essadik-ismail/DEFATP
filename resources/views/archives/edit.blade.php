...
@section('title', 'Éditer archive')

@section('content')
<div class="container mx-auto px-4 py-8">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Éditer l'archive</h1>
            <p class="text-gray-600">Mettre à jour les informations et gérer les documents.</p>
        </div>
    ...
    <form id="archiveEditForm" action="{{ route('archives.update', $archive) }}" method="POST" enctype="multipart/form-data" class="bg-white shadow rounded-lg p-6 space-y-4 mb-6">
        @csrf
        @method('PUT')
                    ...
                    @forelse($archive->documents as $document)
                        <div class="py-3 flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @endif
                                <form action="{{ route('archives.documents.destroy', [$archive, $document]) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:underline" onclick="return confirm('Supprimer ce document ?')">Supprimer</button>
                                </form>
                            </div>
                        </div>
                    @empty
        ...
        <div class="flex items-center justify-between gap-3 pt-4 border-t">
            <a href="{{ route('archives.index') }}" class="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">Retour</a>
            <div class="flex items-center gap-3">
                <button type="button"
                        id="archivePrevBtn"
